'''
Given a string, and a substring, return a new string with all instances of substring
surrounded by underscores.  If overlap exists between multiple instances of substring,
surround the overlapping substrings with only 1 set of underscores.  If the main string
does not contain the substring, return the original string.
'''

# Complexity O(N + M) Time | O(N) Space
# Where N is the length of the string and M is the length of the substring
def underscorifySubstring(string, substring):
    locations = collapse(getLocations(string, substring))
    return underscorify(string, locations)


def getLocations(string, substring):
    locations = []
    startIdx = 0
    while startIdx < len(string):
        nextIdx = string.find(substring, startIdx)
        if nextIdx != -1:
            locations.append([nextIdx, nextIdx + len(substring)])
            startIdx = nextIdx + 1
        else:
            break
    return locations


def collapse(locations):
    if not len(locations):
        return locations
    newLocations = [locations[0]]
    previous = newLocations[0]
    for i in range(1, len(locations)):
        current = locations[i]
        if current[0] <= previous[1]:
            previous[1] = current[1]
        else:
            newLocations.append(current)
            previous = current
    return newLocations


def underscorify(string, locations):
    locationsIdx = 0
    stringIdx = 0
    inBetweenUnderscores = False
    finalChars = []
    i = 0
    while stringIdx < len(string) and locationsIdx < len(locations):
        if stringIdx == locations[locationsIdx][i]:
            finalChars.append("_")
            inBetweenUnderscores = not inBetweenUnderscores
            if not inBetweenUnderscores:
                locationsIdx += 1
            i = 0 if i == 1 else 1
        finalChars.append(string[stringIdx])
        stringIdx += 1
    if locationsIdx < len(locations):
        finalChars.append("_")
    elif stringIdx < len(string):
        finalChars.append(string[stringIdx:])
    return "".join(finalChars)

# First attempt at underscorify - not as readable, but same complexity
# def underscorify(string, locations):
#     newString = []
#     if len(locations) > 0:
#         leftUnderLoc, rightUnderLoc = locations.pop(0)
#     else:
#         leftUnderLoc, rightUnderLoc = None, None
#     for i in range(len(string)):
#         if i == leftUnderLoc:
#             newString.append("_")
#         newString.append(string[i])
#         if i + 1 == rightUnderLoc:
#             newString.append("_")
#             if len(locations) > 0:
#                 leftUnderLoc, rightUnderLoc = locations.pop(0)
#     return "".join(newString)
